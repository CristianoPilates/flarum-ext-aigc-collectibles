      ...
      'browser_run_code',
      {
        code: `async (page) => {
          ...
          function appUrl(pathname) {
            ...
            if (pathname.startsWith('/')) return BASE_URL.replace(/\\/$/, '') + pathname;
              ...
              const created = await app.request({
                ...
                body: {
                  data: {
                    ...
                    attributes: {
                      content: 'create modal inspect bootstrap ' + Date.now(),
          ...
          const button = threadPanel.locator('.Button--primary').filter({ has: page.locator('.fa-handshake') }).first();
          ...
          return await page.evaluate(() => {
            const modal = document.querySelector('.CreateBarterProposalModal');
            const loading = document.querySelector('.CreateBarterProposalModal-loading');
            const columns = Array.from(document.querySelectorAll('.CreateBarterProposalModal-column')).map((node) => node.textContent?.trim() || '');
            const options = Array.from(document.querySelectorAll('.CreateBarterProposalModal-option')).map((node) => node.textContent?.trim() || '');
            const empties = Array.from(document.querySelectorAll('.CreateBarterProposalModal-empty')).map((node) => node.textContent?.trim() || '');

            return {
              modalClass: modal?.className || null,
              modalHtml: modal?.innerHTML?.slice(0, 4000) || null,
...
main().catch((error) => {
  console.error('[mcp-inspect-barter-create-modal] ' + String(error?.stack || error?.message || error));
  process.exit(1);
});
